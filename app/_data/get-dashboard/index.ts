...
export const getDashboard = async (month: string, year?: string) => {
  ...
  const startDate = new Date(`${selectedYear}-${month}-01`);
  const nextMonth = new Date(selectedYear, parseInt(month), 1);
  ...
  const totalExpensePerCategory: TotalExpensePerCategory[] = (
    ...
    percentageOfTotal: expensesTotal
      ? Math.round((Number(category._sum.amount) / Number(expensesTotal)) * 100)
      : 0,
  }));
